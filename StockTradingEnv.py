...
'''
            self.amount=int(self.balance/current_price)
            self.shares=self.amount * current_price
            self.balance -= self.shares
            
            self.trades.append({'step': self.current_step,
                                    'shares': amount, 'total': self.shares,
                                    'type': "buy"})
'''
        elif action_type < 2:
...
'''
            ...
            self.balance+=self.amount*current_price
            self.total_shares_sold += shares_sold
            self.total_sales_value += shares_sold * current_price
            self.trades.append({'step': self.current_step,
                                    'shares': self.amount, 'total': self.amount * current_price,
                                    'type': "sell"})
            self.amount=0
            
        self.net_worth = self.balance + self.amount * current_price
